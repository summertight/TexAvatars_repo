...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : Union[GaussianModel, FlameGaussianModel], load_iteration=None, shuffle=True, resolution_scales=[1.0], org_res=False, use_mask=False):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["DynamicNerf"](args.source_path, args.white_background, args.eval, target_path=args.target_path, use_emo = True, org_res = org_res, use_mask=use_mask)
        ...
        if not self.loaded_iter:
            ...
            print(os.path.join(self.model_path, "input.ply"))
            print(gaussians.ontop)
        ...
        if shuffle:
            ...
            random.shuffle(scene_info.val_cameras)  # Multi-res consistent random shuffling gs
        ...
        if gaussians.ontop != None:
            ...
            self.gaussians.load_extras(scene_info.train_extras, scene_info.test_extras, 
                                    scene_info.tgt_train_extras, scene_info.tgt_test_extras)
        if self.loaded_iter:
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            if camera.mask_path is not None:
                ...
                camera.original_mask = PILtoTorch(mask, (camera.image_width, camera.image_height))
                mask = np.array(mask.convert("L"))
                mask = mask / 255.0
                mask = mask[:, :, np.newaxis]
                arr = arr * mask + camera.bg * (1 - mask)
                image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
            ...
            if resized_image_rgb.shape[1] == 4:
                ...
                image *= gt_alpha_mask
            camera.original_mask_tongue = None
            if camera.mask_tongue_path is not None:
                camera.original_mask_tongue = PILtoTorch(Image.open(camera.mask_tongue_path), (camera.image_width, camera.image_height))
