...
from utils.uv import gen_tritex, calculate_uv_normals
...
FLAME_MODEL_PATH = "flame_model/assets/flame/flame2023.pkl"  
...
def face_vertices(vertices, faces):
    ...
    return vertices[faces.long()] # [batch size, number of faces, 3, 3] 
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        self.num_verts_orig = None
        self.num_faces_orig = None
        ...
        self.register_buffer("vertices", verts.clone(), persistent = False)
        ...
        vertices = np.array(self.v_template) 
        faces = self.faces 

        if add_tongue:
            uv_coords, uv_faces = self.parse_obj_uv("./flame_model/assets/flame/flame_teeth_tongue_unified_v2.obj") 

        else:
            uv_coords, uv_faces = self.parse_obj_uv("./flame_model/assets/flame/final_unified_correct.obj") 

        uv_coords = torch.Tensor(uv_coords) 
        idxim, tidxim, barim, triim = gen_tritex(uv_coords, faces, uv_faces, uv_res) # Generate 512 x 512 texture map.

        self.register_buffer("_idxim", torch.tensor(idxim, dtype=torch.long))
        self.register_buffer("_barim", torch.tensor(barim))
        ...
        xs = torch.linspace(-1, 1, steps= sample_res)
        ys = torch.linspace(-1, 1, steps= sample_res)

        xs, ys = torch.meshgrid(xs, ys, indexing='ij')
        sampled_uv_coords = torch.stack([ys, xs], dim=-1)

        torch_position_map = torch.from_numpy(flame_position_map).float().permute(2, 0, 1) 
        self.torch_position_map = torch_position_map
        torch_face_index_map = torch.from_numpy(idxim).permute(2, 0, 1)
        valid_uv_map = (torch_face_index_map >= 0).any(dim=0).float()[None] 
        ...
        valid_uv_coords = sampled_uv_coords[valid_samples] 

        torch_faces_index_map = torch.from_numpy(triim).float()
        self.register_buffer("_triim", torch_faces_index_map) 
        self.valid_mask = (self._triim) != -1
        uv_grid = valid_uv_coords.unsqueeze(0).unsqueeze(2)
        self.register_buffer("_uv_grid", uv_grid.contiguous())

        self.register_buffer("_uv_position_map", torch_position_map.contiguous()) 

        # [Valid 3D (barycentric-interpolated) Points Sampling]
    ...
    def parse_obj_uv(self, file_path):
        """
        Parses an .obj file to extract UV coordinates and face UV indices.

        Args:
        - file_path (str): The path to the .obj file.

        Returns:
        - uv_verts (list of tuples): List of UV coordinates (u, v).
        - uv_faces (list of lists): List of lists, each containing UV indices for a face.
        """
        uv_verts = []  # UV coordinates (vt)
        uv_faces = []  # UV face indices (f)
        ...
        with open(file_path, "r") as file:
            for line in file:
                ...
                if parts[0] == "vt":  # UV coordinates (vt)
                    u, v = float(parts[1]), float(parts[2])
                    uv_verts.append((u, v))

                elif parts[0] == "f":  # Face indices (f)
                    face_uv_indices = []
                    for vertex in parts[1:]:
                        v_info = vertex.split('/')
                        if len(v_info) > 1 and v_info[1]:  # Ensure vt index exists
                            face_uv_indices.append(int(v_info[1]) - 1)  # Convert to 0-based index
                    uv_faces.append(face_uv_indices)
    ...
    def add_tongue(self):
        ...
        tongue_mesh_path = 'flame_model/assets/flame/tongue_wellpositioned_0506.obj'
        v_tongue, faces_tongue, aux_tongue = load_obj(tongue_mesh_path)

        f_tongue = faces_tongue.verts_idx

        num_verts_tongue = v_tongue.shape[0]
        self.num_verts_tongue = num_verts_tongue

        self.v_template = torch.cat([self.v_template, v_tongue], dim=0)
        num_verts_full = self.v_template.shape[0]
        self.mask.v.register_buffer("tongue", torch.arange(num_verts_orig, num_verts_orig+num_verts_tongue))
        ...
        self.faces_original = self.faces.clone()
        self.faces = torch.cat([self.faces, num_verts_orig + f_tongue], dim=0)

        self.faces_tongue_local = f_tongue
        self.vids_tongue = torch.arange(num_verts_orig, num_verts_orig + num_verts_tongue)
        
        self.mask.num_verts = num_verts_full

        self.lbs_weights = torch.cat([self.lbs_weights, torch.zeros_like(self.lbs_weights[:num_verts_tongue])], dim=0)  
        self.lbs_weights[self.vids_tongue, 2] += 1  # move with jaw

        verts_uvs_tongue = aux_tongue.verts_uvs

        self.verts_uvs = torch.cat([self.verts_uvs, verts_uvs_tongue], dim=0)
        num_verts_uv_orig = self.verts_uvs.shape[0]
        self.textures_idx = torch.cat([self.textures_idx, num_verts_uv_orig + f_tongue], dim=0)
    ...
    def add_teeth(self):
        ...
        num_verts_orig = self.v_template.shape[0]
        self.num_faces_orig = self.faces.size(0)
        ...
        v_teeth = torch.cat([
            ...
            v_teeth_upper_edge, 
            v_teeth_lower_edge, 
            v_teeth_upper_root_back,  
            v_teeth_upper_edge_back,  
            v_teeth_lower_root_back, 
            v_teeth_lower_edge_back,  
        ], dim=0)
        num_verts_teeth = v_teeth.shape[0]
        self.register_buffer("teeth_v", v_teeth.clone(), persistent = False) 
        self.register_buffer("v_template_without_teeth", self.v_template.clone(), persistent = False)
        self.v_template = torch.cat([self.v_template, v_teeth], dim=0)

        self.vertices = torch.cat([self.vertices, v_teeth], dim=0)
        ...
        self.register_buffer("faces_without_teeth", self.faces.clone(), persistent = False) # by justin
        self.faces = torch.cat([self.faces, f_teeth_upper+num_verts_orig, f_teeth_lower+num_verts_orig], dim=0)

        self.num_verts_orig = num_verts_orig
        self.register_buffer("teeth_f", torch.cat([f_teeth_upper, f_teeth_lower]), persistent = False) #by justin
        self.register_buffer("faces_with_teeth", self.faces.clone(), persistent = False) # by justin
