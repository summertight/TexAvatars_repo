...
def gen_tritex(vt: np.ndarray, vi: np.ndarray, vti: np.ndarray, texsize: int):
    ...
    viim = np.full((texsize, texsize, 3), -1, dtype=np.int32)
    vtiim = np.full((texsize, texsize, 3), -1, dtype=np.int32)
    baryim = np.full((texsize, texsize, 3), -1.0, dtype=np.float32)
    triim = np.full((texsize, texsize), -1, dtype=np.int32)
    ...
    for i in list(range(ntris))[::-1]: 
        ...
        if denom != 0.: 
            ...
            triim[bbox[2]:bbox[3], bbox[0]:bbox[1]] = np.where( 
                ((baryu >= 0.) & (baryv >= 0.) & (baryw >= 0.))[:, :], 
                i, 
                triim[bbox[2]:bbox[3], bbox[0]:bbox[1]])

    return viim, vtiim, baryim, triim

def calculate_tbn_uv(xyz, return_scale=False):
    gradients = spatial_gradient(xyz) 
    t, b = gradients[:, :, 0], gradients[:, :, 1] 
    normals = F.normalize(torch.cross(t, b, dim=1), dim=1, p=2)
    bitangent = F.normalize(torch.cross(t, normals, dim=1), dim=1, p=2)
    tangent = F.normalize(torch.cross(bitangent, normals, dim=1), dim=1, p=2)

    
    R = torch.stack([tangent.permute(0, 2, 3, 1), bitangent.permute(0, 2, 3, 1), normals.permute(0, 2, 3, 1)], dim=-1)
    if not return_scale:
        return R
    S = torch.norm(torch.cross(t, b, dim=1), dim=1) / 2 
    return R, S

def calculate_uv_normals(xyz):
    gradients = spatial_gradient(xyz) 
    t, b = gradients[:, :, 0], gradients[:, :, 1] 
    normals = F.normalize(torch.cross(t, b, dim=1), dim=1, p=2)
    return normals.permute(0, 2, 3, 1)

def deformation_uv(xyz, return_scale = False):
    B, C, H, W = xyz.shape
    RT = torch.zeros([B, H, W, 4, 4]).cuda().float()
    RT[:, :, :, 3, 3] = 1
    if return_scale:
        R, S = calculate_tbn_uv(xyz, return_scale = return_scale)
    else:
        R = calculate_tbn_uv(xyz, return_scale = return_scale)
